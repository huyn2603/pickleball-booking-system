...
* **Kiến Trúc:** Mô hình Monorepo với ranh giới phân tách rõ ràng:
  * `backend/`: Sử dụng Java, xử lý logic nghiệp vụ cốt lõi, kiểm tra tính hợp lệ (validation) và cung cấp các RESTful API.
...
* **Backend:** Java (Khuyến khích sử dụng Spring Boot Framework)
* **Frontend:** ReactJS, HTML5, CSS3, JavaScript
* **Cơ sở dữ liệu:** MongoDB
* **Tích hợp:** Cổng thanh toán (VNPay/MoMo), Dịch vụ gửi Email, Tác vụ lập lịch tự động hủy (Auto-Cancel Cron Jobs / Scheduled Tasks).
...
* **Khóa Slot Tạm Thời (Slot Locking):** Tạm thời giữ khung giờ khi khách hàng thêm sân vào giỏ hàng.
...
* **Phân Tách Lớp Ở Backend:** Tuân thủ mô hình Pattern: Controller -> Service -> Repository. Controller xử lý điều hướng/kiểm tra dữ liệu đầu vào; toàn bộ logic nghiệp vụ phải nằm ở lớp Service.
...
* **Bảo Mật:** Không viết cứng (hardcode) chuỗi kết nối MongoDB hoặc JWT secret vào mã nguồn. Sử dụng biến môi trường (environment variables) hoặc các file cấu hình bảo mật.
